...
const name = "Galuh Test Transaction";
const email = "galuhtesttransaction@dev.com";
...
describe("Testing Transactions endpoint", () => {
  ...
  describe("Tetsing get detail transaction GET /api/v1/transactions/{transactionId} endpoint ", () => {
    test("should can get detail transactions", async () => {
      const { statusCode, body } = await request(app).get(
        `/api/v1/transactions/${transactionTemp.id}`
      );
      expect(statusCode).toBe(200);
      expect(body).toHaveProperty("success");
      expect(body).toHaveProperty("message");
      ...
      expect(body.data.source_account_id).toBe(
        transactionTemp.source_account_id
      );
      expect(body.data.destination_account_id).toBe(
        transactionTemp.destination_account_id
      );
    });

    test("should can't get detail transactions tranasactionId not found", async () => {
      const { statusCode, body } = await request(app).get(
        `/api/v1/transactions/${transactionTemp.id}99`
      );
      expect(statusCode).toBe(404);
      expect(body).toHaveProperty("success");
      expect(body).toHaveProperty("message");
